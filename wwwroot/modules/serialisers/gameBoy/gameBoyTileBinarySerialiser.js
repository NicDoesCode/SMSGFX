...
export default class GameBoyTileBinarySerialiser {
    ...
    static deserialise(planarByteArray) {
        ...
        for (let i = 0; i < planarByteArray.length; i += 2) {
            const byte0 = planarByteArray[i + 0];
            const byte1 = planarByteArray[i + 1];
            const px0 = ((byte1 & masks[0]) >> 6) | ((byte0 & masks[0]) >> 7);
            const px1 = ((byte1 & masks[1]) >> 5) | ((byte0 & masks[1]) >> 6);
            const px2 = ((byte1 & masks[2]) >> 4) | ((byte0 & masks[2]) >> 5);
            const px3 = ((byte1 & masks[3]) >> 3) | ((byte0 & masks[3]) >> 4);
            const px4 = ((byte1 & masks[4]) >> 2) | ((byte0 & masks[4]) >> 3);
            const px5 = ((byte1 & masks[5]) >> 1) | ((byte0 & masks[5]) >> 2);
            const px6 = ((byte1 & masks[6]) >> 0) | ((byte0 & masks[6]) >> 1);
            const px7 = ((byte1 & masks[7]) << 1) | ((byte0 & masks[7]) >> 0);
